...
## Расчёт надёжного вывода из интерфейса

На вкладке «Надёжный вывод» находится кнопка «Рассчитать надёжный вывод».
Она запускает расширенную проверку для выбранного спектра в отдельном потоке:

- повторяет подбор с несколькими начальными приближениями;
- проверяет устойчивость выбора семейства и схемы на повторных выборках;
- проверяет распределение времён релаксации;
- применяет критерии допустимости данных и диффузионного механизма;
- автоматически заполняет вкладку итоговым решением.

Расчёт использует выбранный канал импеданса. При новом подборе или смене канала
предыдущий надёжный вывод сбрасывается, поскольку он относится к прежним данным.
Ручной импорт полного результата в формате JSON сохранён для воспроизводимости
и переноса результатов между машинами.
